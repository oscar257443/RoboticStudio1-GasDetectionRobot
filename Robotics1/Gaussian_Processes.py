...
def gaussianroboticstudio(filename,std):
    ...
    # yq = rng.multivariate_normal(mean_q, cov_qq, 3).T   # samples that follow the gaussian process ie can be possible
    ...
    plt.plot(xq, mean_q, 'k', label="mean")
    plt.fill_between(xq, mean_q + std_q, mean_q - std_q, color='k', alpha=0.3, label="std dev")
    plt.scatter(max_time, max_value, c='g', marker='o', s=100, label="Max Value")
    plt.xlim(max_time - 2, max_time + 2)
    plt.ylim(max_value - 2 * std_q[max_index], max_value + 2 * std_q[max_index])
    plt.legend()
    plt.xlabel("Time")
    plt.ylabel("PPM")
    plt.title("At max value")
...
gaussianroboticstudio('Safe.csv',0.5)
gaussianroboticstudio('Dangerous.csv',0.5)
gaussianroboticstudio('Dangerous_gaps.csv',0.5)
# gaussianroboticstudio('Test.csv',0.5)
